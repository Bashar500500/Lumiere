...
class LearningActivityRequest extends FormRequest
{
    ...
    protected function onStore() {
        return [
            ...
            'status' => ['required', new Enum(LearningActivityStatus::class)],
            'flags' => ['required', 'array'],
            'flags.isFreePreview' => ['required', 'boolean'],
            'flags.isCompulsory' => ['required', 'boolean'],
            'flags.requiresEnrollment' => ['required', 'boolean'],
            'content' => ['required', 'array'],
            'content.type' => ['required', new Enum(LearningActivityContentType::class), 'same:type'],
            'content.data' => ['required', 'array'],
            'content.data.pdf' => ['sometimes', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'file', 'mimes:pdf'],
            'content.data.sizeMB' => ['required_if:content.type,==,PDF', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'integer', 'gt:0'],
            'content.data.pages' => ['required_if:content.type,==,PDF', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'integer', 'gt:0'],
            'content.data.watermark' => ['sometimes', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'boolean'],
            'content.data.video' => ['sometimes', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Video->getEnumsExceptValue()), 'file', 'mimes:mp4,mov,ogg,qt,ogx,oga,ogv,webm'],
            'content.data.duration' => ['required_if:content.type,==,Video', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Video->getEnumsExceptValue()), 'integer', 'gt:0'],
            'content.data.captions' => ['sometimes', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Video->getEnumsExceptValue()), 'array'],
            'content.data.captions.language' => ['required_with:content.data.captions', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Video->getEnumsExceptValue()), 'string'],
            'content.data.captions.url' => ['required_with:content.data.captions', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Video->getEnumsExceptValue()), 'url'],
            'thumbnailUrl' => ['sometimes', 'url'],
            'completion' => ['required', 'array'],
            'completion.type' => ['required', new Enum(LearningActivityCompletionType::class)],
            ...
            'completion.passingScore' => ['required_if:completion.type,==,Score', 'missing_if:content.type,==,Score' . implode(',', LearningActivityCompletionType::Score->getEnumsExceptValue()), 'integer', 'gt:0'],
            'completion.rules' => ['required_if:completion.type,==,Composite', 'missing_if:content.type,==,Composite' . implode(',', LearningActivityCompletionType::Composite->getEnumsExceptValue()), 'array'],
            ...
            'availability.timezone' => ['required_with:availability', 'regex:^(?:Z|[+-](?:2[0-3]|[01][0-9]):[0-5][0-9])$^'],
            'discussion' => ['sometimes', 'array'],
            'discussion.enabled' => ['required_with:discussion', 'boolean'],
            'discussion.moderated' => ['required_with:discussion', 'boolean'],
    ...
    protected function onUpdate() {
        return [
            ...
            'content.type' => ['required_with:type', new Enum(LearningActivityContentType::class), 'same:type'],
            'content.data' => ['required', 'array'],
            'content.data.pdf' => ['sometimes', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'file', 'mimes:pdf'],
            'content.data.sizeMB' => ['required_if:content.type,==,PDF', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'integer', 'gt:0'],
            'content.data.pages' => ['required_if:content.type,==,PDF', 'missing_if:content.type,==,' . implode(',', LearningActivityContentType::Pdf->getEnumsExceptValue()), 'integer', 'gt:0'],
            ...
            'completion.rules' => ['required_if:completion.type,==,Composite', 'missing_if:content.type,==,Composite' . implode(',', LearningActivityCompletionType::Composite->getEnumsExceptValue()), 'array'],
            'availability' => ['sometimes', 'array'],
            'availability.start' => ['sometimes', 'date', 'date_format:Y-m-d'],
            'availability.end' => ['required_with:availability.start', 'date', 'date_format:Y-m-d', 'after_or_equal:availability.start'],
